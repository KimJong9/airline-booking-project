...
function BookingSection() {
    ...
    // 출발지 나라 목록 불러오기
    useEffect(() => {
        axios.get('http://localhost:5001/search/countries')
    ...
    useEffect(() => {
        if (selectedDepartureCountry) {
            axios.get(`http://localhost:5001/search/cities/${selectedDepartureCountry}`)
    ...
    useEffect(() => {
        if (selectedArrivalCountry) {
            axios.get(`http://localhost:5001/search/cities/${selectedArrivalCountry}`)
    ...
    useEffect(() => {
        if (selectedDepartureCity) {
            axios.get(`http://localhost:5001/search/airports/${selectedDepartureCountry}/${selectedDepartureCity}`)
    ...
    useEffect(() => {
        if (selectedArrivalCity) {
            axios.get(`http://localhost:5001/search/airports/${selectedArrivalCountry}/${selectedArrivalCity}`)
    ...
    const handleSearch = () => {
        ...
        axios.get(`http://localhost:5001/search/flights`, {
            params: {
